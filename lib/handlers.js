  ...
  if (email) {
    _data.read('cart', email, (err, cartData) => {
      if (!err && cartData) {
        ...
        handlers._tokens.verifyToken(token, email, (tokenIsValid) => {
          if (tokenIsValid) {
            _data.update('cart', email,[], (err) => {
              if (!err) {
                callback(false)
              } else {
                callback(500, { 'Error': 'Some error while clearing file ' + email + '.json' })
              }
            })
          } else {
            callback(403, { 'Error': 'Missing required token in header, or token is invalid' })
  ...
  if (email) {
    ...
    _data.read('cart', email, (err, cartData) => {
      if (!err && cartData) {
        ...
        if(cartData.length == 0) callback(400, { 'Error': 'No Items in cart to order' })
        // Get the token from the Headers
        const token = typeof (data.headers.token) == 'string' ? data.headers.token : false
          ...
          if (!err && tokenData) {
            ...
            handlers._tokens.verifyToken(token, email, (tokenIsValid) => {
              if (tokenIsValid) {
                ...
                helpers.makeStripeTransaction(email, total, (statusTransaction, responseTransaction) => {
                  if ([200, 201].indexOf(statusTransaction) == -1) {
                    ...
                    helpers.sendMailgunEmail(email, receiptUrl, (statusEmail, responseEmail) => {
